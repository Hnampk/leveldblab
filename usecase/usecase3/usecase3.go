...
func LevelDBNormalTesting(read int, write int, duration time.Duration) {

	envRootFolder := os.Getenv("RootFolder")
	if envRootFolder != "" {
		rootFolder = envRootFolder + ""
	}
	dbFile, err := db.NewLevelDBNormal(path.Join(rootFolder, "usecase3"))
	if err != nil {
